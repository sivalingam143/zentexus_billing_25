...
import {
  ...
  updateringboxing,
  deleteringboxing,
} from "../../slice/RingboxingsectionSlice";
...
import RingboxingCreation from "../creations/RingboxingCreation";
...
const RingBoxing = () => {
  ...
  const [filteredRingboxing, setFilteredRingboxing] = useState([]);

  const productOptions = [
    { label: "சோர்ச்சா", value: "சோர்ச்சா" },
    { label: "ஜயன்ட்", value: "ஜயன்ட்" },
    { label: "திரி சோர்ச்சா", value: "திரி சோர்ச்சா" },
    { label: "திரி ஜயன்ட்", value: "திரி ஜயன்ட்" },
  ];

  // RingboxingCreation.js ல இருந்து staffOptions லாஜிக்
  ...
  const handleApplyFilters = (e) => {
    ...
    setFilteredRingboxing(filteredData);
    setFilterShow(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (editMode) {
      ...
      try {
        await dispatch(
          updateringboxing({ id: selectedEntry.id, ...formData })
        ).unwrap();
      ...
      try {
        await dispatch(addringboxing(formData)).unwrap();
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deleteringboxing(id)).unwrap();
  ...
  const data =
    filteredRingboxing?.length > 0
      ...
      ringboxing?.length > 0
        ? ringboxing
            .filter((entry) => entry.entry_date === today) // Filter entries for today's date
            .map((entry, index) => {
              let products = [];
              try {
                products =
  ...
  return (
    <div id="main">
      <Container>
        <Row>
          <Col xs="6" className="py-3">
            <PageTitle PageTitle="வளையம் குத்து" showButton={false} />
          </Col>
          <Col
            xs="6"
            md="6"
          ...
          <Col xs="12" className="py-3">
            <TableUI
              headers={headers}
              body={filteredRingboxing.length > 0 ? data : data1}
            />
          </Col>
        </Row>
      </Container>
      <CustomModal
        ...
        setShow={setShow}
        pageTitle={
          editMode ? <>வளையம் குத்து திருத்து</> : <>வளையம் குத்து உருவாக்கு</>
        }
        showButton={true}
        submitButton={true}
      ...
      <Offcanvas show={filterShow} onHide={handleFilterClose} placement="end">
        <Offcanvas.Header closeButton>
          <Offcanvas.Title>வளையம் குத்து Filter</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          <Form>
