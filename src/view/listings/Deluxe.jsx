...
import {
  ...
  updatedeluxe,
  deletedeluxe,
} from "../../slice/DeluxeSlice";
import { HiOutlineDotsVertical } from "react-icons/hi";
import CustomModal from "../../components/Modal";
import NotifyData from "../../components/NotifyData";
import PageTitle from "../../components/PageTitle";
import DeluxeCreation from "../creations/DeluxeCreation";
import { fetchStaff } from "../../slice/StaffSlice";
import { fetchProduct } from "../../slice/ProductSlice";

const Deluxe = () => {
  const dispatch = useDispatch();
  const { deluxe, status, error } = useSelector((state) => state.deluxe);
  ...
  const [filteredDeluxe, setFilteredDeluxe] = useState([]);

  const ProductOptions = Product.filter(
    (product) =>
      product.deluxe_Knitting_wage &&
      product.deluxe_Knitting_wage !== "" &&
      product.deluxe_Knitting_wage !== null
  ).map((product) => ({
    label: product.product_name,
    value: product.product_name,
  }));

  // DeluxeCreation.js ல இருந்து staffOptions லாஜிக்
  const staffOptions = Staff.filter(
    (staffMember) =>
      Array.isArray(staffMember.staff_type) &&
      staffMember.staff_type.includes("டிலெஸ் பின்னல்")
  ).map((staffMember) => ({
  ...
  const handleApplyFilters = (e) => {
    ...
    setFilteredDeluxe(filteredData);
    setFilterShow(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (editMode) {
      ...
      try {
        await dispatch(
          updatedeluxe({ id: selectedEntry.id, ...formData })
        ).unwrap();
      ...
      try {
        await dispatch(adddeluxe(formData)).unwrap();
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deletedeluxe(id)).unwrap();
  ...
  const data =
    filteredDeluxe?.length > 0
      ? filteredDeluxe.map((entry, index) => {
          let products = [];
          try {
            products =
              typeof entry.products === "string"
  ...
  const data1 =
    deluxe?.length > 0
      ? deluxe.filter((entry)=> entry.entry_date === today).map((entry, index) => {
          let products = [];
          try {
            products =
              typeof entry.products === "string"
  ...
  return (
    <div id="main">
      <Container>
        <Row>
          <Col xs="6" className="py-3">
            <PageTitle PageTitle="டீலக்ஸ் பின்னல்" showButton={false} />
          </Col>
          <Col
            xs="6"
            md="6"
          ...
          <Col xs="12" className="py-3">
            ...
            <TableUI
              headers={headers}
              body={filteredDeluxe.length > 0 ? data : data1}
            />
          </Col>
        </Row>
      </Container>
      <CustomModal
        ...
        pageTitle={
          editMode ? (
            <>டீலக்ஸ் பின்னல் திருத்து</>
          ) : (
            <>டீலக்ஸ் பின்னல் உருவாக்கு</>
          )
        }
        showButton={true}
        submitButton={true}
      ...
      <Offcanvas show={filterShow} onHide={handleFilterClose} placement="end">
        <Offcanvas.Header closeButton>
          <Offcanvas.Title>டீலக்ஸ் பின்னல் Filter</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          <Form>
