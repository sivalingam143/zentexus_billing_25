...
const SaleCreation = ({ tabNumber = 1 }) => {
  ...
  return products.map((p) => {
    ...
    try {
      ...
      const pp = JSON.parse(p.purchase_price || "{}");
      purchasePrice = pp.price || "0";
    } catch (e) {}

    return {
      value: p.product_name,
      label: `${p.product_name} - Sale: ₹${salePrice}, Purchase: ₹${purchasePrice}`, // Update label
      product_id: p.product_id,
      hsn_code: p.hsn_code || "",
      salePrice: salePrice,
...
<Table bordered hover className="mb-0">
  ...
  <tbody>
    {products
      ...
      .map((product) => {
        let salePrice = "0";
        let purchasePrice = "0"; // Add this variable
        let stock = "0";
        let location = "-";
