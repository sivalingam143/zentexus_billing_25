...
const ProformaCreation = ({ tabNumber = 1 }) => {
  ...
  const [tabs, setTabs] = useState([
    { id: 1, title: "Proforma#1", active: true }
  ]);
  ...
  const { parties = [], partiesStatus = "idle", Proforma = [] } = ProformaState || {};
  ...
  const isEditMode = location.pathname.startsWith("/Proforma/edit");
  const isViewMode = location.pathname.startsWith("/Proforma/view");
  const isCreateMode = location.pathname === "/Proforma/create";
  ...
  const ProformaToEdit = id ? Proforma.find((e) => e.Proforma_id == id) : null;
  const [firstProformaNo, setFirstProformaNo] = useState(null);
  ...
  const addNewTab = () => {
    ...
    const allProformaNumbers = [];
    
    Object.values(tabForms).forEach(tabForm => {
      if (tabForm.Proforma_no) {
        allProformaNumbers.push(tabForm.estimate_no);
      }
    });
    
    // Add current active tab's invoice number
    if (formData.Proforma_no && formData.Proforma_no !== "Generating...") {
      allProformaNumbers.push(formData.Proforma_no);
    }
    
    const currentYearMonth = "EST" + new Date().toISOString().slice(0, 7).replace(/-/g, "");
    
    let nextProformaNumber = "0001";
    
    if (allProformaNumbers.length > 0) {
      const currentMonthProforma = allProformaNumbers.filter(est => 
        est?.startsWith(currentYearMonth)
      );
      
      if (currentMonthProforma.length > 0) {
        const numbers = currentMonthProforma.map((est) => {
          const numPart = est.split("-")[1];
          return parseInt(numPart || "0");
        });
        const maxNum = Math.max(...numbers);
        nextProformaNumber = String(maxNum + 1).padStart(4, "0");
      }
    }
    
    const nextProformaNo = `${currentYearMonth}-${nextProformaNumber}`;
   ...
   setTabs(prev => [
      ...
      { id: newTabId, title: `Proforma#${newTabId}`, active: true }
    ]);
  ...
  useEffect(() => {
    if (ProformaToEdit) {
      let itemsArray = [];
      try {
        itemsArray = JSON.parse(ProformaToEdit.products || "[]");
      } catch (e) {
  ...
  useEffect(() => {
    if (isCreateMode) {
      dispatch(searchProforma("")).then((action) => {
        if (action.payload && action.payload.length > 0) {
          const allProforma = action.payload.map((Proforma) => Proforma.Proforma_no);
          const currentYearMonth =
            "EST" + new Date().toISOString().slice(0, 7).replace(/-/g, "");

          const currentMonthProforma = allProforma.filter((est) =>
            est?.startsWith(currentYearMonth)
          );
          ...
          const nextProformaNo = `${currentYearMonth}-${String(nextNum).padStart(
            4,
            "0"
          )}`;
          ...
          const currentYearMonth =
            "EST" + new Date().toISOString().slice(0, 7).replace(/-/g, "");
          const firstProforma = currentYearMonth + "-0001";
          setFirstProformaNo(firstProforma);
          setFormData((prev) => ({
            ...prev,
            Proforma_no: firstProforma,
          }));
  ...
  // FetchProforma for edit/view
  useEffect(() => {
    if ((isEditMode || isViewMode) && Proforma.length === 0) {
      dispatch(searchProforma(""));
    }
  }, [isEditMode, isViewMode, Proforma.length, dispatch]);
  ...
  useEffect(() => {
    ...
    console.log("Loading edit data for Proforma:", ProformaToEdit);
    ...
    // Set party selection
    if (ProformaToEdit.parties_id) {
      const partyFromList = parties.find(
        (p) =>
          p.id == ProformaToEdit.parties_id || p.parties_id == ProformaToEdit.parties_id
      );
      if (partyFromList) {
    ...
    const round_off = Number(ProformaToEdit.round_off || 0);
    const round_off_amount = String(ProformaToEdit.round_off_amount || "0");
    ...
    setImagePreview(ProformaToEdit.add_image || "");
    setImageFileName("");
    setHasUserUploadedImage(false);

    // Parse documents
    let docs = [];
    if (ProformaToEdit.documents) {
      try {
        docs = JSON.parse(ProformaToEdit.documents);
      } catch (e) {
    ...
    setFormData({
      parties_id: ProformaToEdit.parties_id || "",
      name: ProformaToEdit.name || "",
      phone: ProformaToEdit.phone || "",
      billing_address: ProformaToEdit.billing_address || "",
      shipping_address: ProformaToEdit.shipping_address || "",
      Proforma_no: ProformaToEdit.Proforma_no || "", 
      Proforma_date: 
       ProformaToEdit.Proforma_date || new Date().toISOString().split("T")[0],
      state_of_supply: ProformaToEdit.state_of_supply || "",
      payment_type: ProformaToEdit.payment_type || "",
      description: ProformaToEdit.description || "",
      add_image: ProformaToEdit.add_image || "",
      rows,
      round_off,
      round_off_amount,
      total,
      received_amount: ProformaToEdit.received_amount || " ",
      visibleColumns: {
        category: hasCategory,
        description: hasDescription,
        hsn_code: hasHsn,
    ...
    setTabForms(prev => ({
      ...
      1: {
        ...
        shipping_address: ProformaToEdit.shipping_address || "",
       Proforma_no: ProformaToEdit.Proforma_no || "", 
        Proforma_date: ProformaToEdit.Proforma_date || new Date().toISOString().split("T")[0],
        state_of_supply: ProformaToEdit.state_of_supply || "",
        payment_type: ProformaToEdit.payment_type || "",
        description: ProformaToEdit.description || "",
        add_image: ProformaToEdit.add_image || "",
        ...
        _selectedPartyOption: selectedPartyOption,
        _imagePreview: ProformaToEdit.add_image || "",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        Proforma_no: formData.Proforma_no,
      ...
      delete payload.rows;
      delete payload.visibleColumns; 

      
      if (isEditMode) {
        payload.edit_Proforma_id = id;
      }

      console.log("Sending payload:", payload);

      if (isEditMode) {
        await dispatch(updateProforma(payload)).unwrap();
        NotifyData("Proforma Updated Successfully!", "success");
      } else {
        await dispatch(createProforma(payload)).unwrap();
        NotifyData("Proforma Created Successfully!", "success");
      }
      ...
      NotifyData(err?.message || "Failed to save Proforma", "error");
    }
  };
  

  const handleBack = () => navigate("/Proforma");
      ...
      {isViewMode ? (
        ...
        />
              ...
              <Col md={2} style={{ zIndex: 100 }}>
                <TextInputform
                  formLabel="Proforma No"
                  value={formData.Proforma_no || "Generating..."}
                  readOnly={true}
                  style={{
                ...
                />
                <Calender
                  calenderlabel="Proforma Date"
                  initialDate={formData.Proforma_date}
                  readOnly={isDisabled}
                />
                <DropDown
...
export default ProformaCreation;