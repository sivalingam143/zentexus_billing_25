...
import {
  ...
  updateknotting,
  deleteknotting,
} from "../../slice/KnottingSlice";
import { fetchStaff } from "../../slice/StaffSlice";
import { HiOutlineDotsVertical } from "react-icons/hi";
import CustomModal from "../../components/Modal";
import NotifyData from "../../components/NotifyData";
import PageTitle from "../../components/PageTitle";
import KnotCreation from "../creations/KnotCreation";

const Knotting = () => {
  const dispatch = useDispatch();
  const { knotting, status, error } = useSelector((state) => state.knotting);
  ...
  const [filteredknotting, setFilteredknotting] = useState([]);

  const productOptions = Product.filter(
    (product) =>
      product.Knitting_wage &&
      product.Knitting_wage !== "" &&
      product.Knitting_wage !== null
  ).map((product) => ({
    label: product.product_name,
    value: product.product_name,
  }));

  // KnotCreation.js ல இருந்து staffOptions லாஜிக்
  const staffOptions = Staff.filter(
    (staffMember) =>
      Array.isArray(staffMember.staff_type) &&
      staffMember.staff_type.includes("பின்னல்")
  ).map((staffMember) => ({
  ...
  const handleApplyFilters = (e) => {
    ...
    setFilteredknotting(filteredData);
    setFilterShow(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (editMode) {
      ...
      try {
        await dispatch(
          updateknotting({ id: selectedEntry.id, ...formData })
        ).unwrap();
      ...
      try {
        await dispatch(addknotting(formData)).unwrap();
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deleteknotting(id)).unwrap();
  ...
  const data1 =
    knotting?.length > 0
      ? knotting.filter((entry) => entry.entry_date === today).map((entry, index) => {
          let products = [];
          try {
            products =
              typeof entry.products === "string"
  ...
  return (
    <div id="main">
      <Container>
        <Row>
          <Col xs="6" className="py-3">
            <PageTitle PageTitle="பின்னல்" showButton={false} />
          </Col>
          <Col
            xs="6"
            md="6"
          ...
          <Col xs="12" className="py-3">
            ...
            <TableUI
              headers={headers}
              body={filteredknotting.length > 0 ? data : data1}
            />
          </Col>
        </Row>
      </Container>
      <CustomModal
        show={show}
        setShow={setShow}
        pageTitle={editMode ? <>பின்னல் திருத்து</> : <>பின்னல் உருவாக்கு</>}
        showButton={true}
        submitButton={true}
